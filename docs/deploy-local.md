...
メンテナ向け。**qarows** Local 版（`apps/local`）を公式 Cloudflare Pages インスタンスとして公開する手順。
...
| 項目 | 内容 |
|---|---|
| 成果物 | `apps/local/dist`（Vite ビルドの静的 SPA） |
| ホスティング | Cloudflare Pages（Git 連携） |
| 本番 URL | **https://qarows.fuyuz.dev** |
| 本番ブランチ | `main` |
| ルーティング | `apps/local/public/_redirects` → SPA フォールバック |
...
| **Root directory (advanced) → Path** | **`apps/local`**（monorepo 用。ここが重要） |
| Build command | `bun run build` |
| Build output directory | **`dist`**（Vite preset なら自動。Path を `apps/local` にした場合の相対パス） |

Root directory を `apps/local` にすると、成果物は `apps/local/dist` に出力され、Cloudflare は `dist` を参照する。リポジトリルートのまま使う場合は output を `apps/local/dist` にする（項目が見えない場合は Root directory 方式を推奨）。
...
リグレッション用チェックリストは `apps/local/public/samples/tests.yml` のシナリオを参照。
...
| 症状 | 対処 |
|---|---|
| ビルド失敗 | ローカルで `bun run build` が通るか確認。output path が `apps/local/dist` か確認 |
...
bunx wrangler login
bun run build
bunx wrangler pages deploy apps/local/dist --project-name=qarows --branch=main
```
