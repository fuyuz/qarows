...
`bug.id` でマージする。片方にしかないバグは両方残る。同一 id が両方にある場合はフィールド単位で解決する。

| フィールド | ルール |
|---|---|
| `title` / `severity` / `status` | 後から読み込んだ側を採用（必須フィールド） |
| `memo` / `fixNote` | 両方残す（改行連結、`---` 区切り）。`---` 区切りのセグメント単位で重複を除くので、同じファイルを繰り返しマージしても増えない |
| `environmentIds` | 両方の和（重複除去）。別端末で踏んだ報告を落とさない |
| `testCaseId` / `assignee` / `steps` / `expected` / `actual` / `attachments` | 値があるほうを採用（後勝ち。片方が未設定なら既存値を保持） |

省略されたフィールドで既存値を消さない。片方の `results.json` が任意フィールドを持たないだけで
添付やメモが失われるのを避けるためで、逆に「意図的に空にした」操作は復活しうる（データ欠落より安全側に倒す）。

`attachments` は和を取らない。parse 時に `MAX_BUG_ATTACHMENTS` で切られる一方、マージ結果は再正規化されないため。

必須フィールドが後勝ちである点は意図的で、**結果ステータスの `OK < SKIP < NG`（強い方が勝つ）とは別ルール**。
`bug.status` / `bug.severity` に強弱の順序を定義していないため、`Bug` に更新時刻がない現状では順序でしか決められない。
そのため古い `results.json` を後にマージすると、バグの状態や重要度が巻き戻ることがある（マージ順に注意）。
