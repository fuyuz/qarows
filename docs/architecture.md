...
| `packages/runner-ui` | テスト実行 UI（FilterBar、TestRunner、進捗バー等）。Phase 1 / 2 で同一レイアウト |
...
- **Phase 1**: `IndexedDbProjectRepository` + `LocalProjectChannel`（即時適用）
- **Phase 2**: `HttpProjectRepository` + `WebSocketProjectChannel`（DO へ command 送信）

`ProjectCommand` の例: `updateResult`, `updateResultsBatch`, `setSession`, `updateTestCase`, `addBug`, `mergeResults`（Phase 1 のみ） 等。
...
利用者各自が Cloudflare アカウントに **セルフデプロイ** する closed 環境。同一デプロイ内のメンバーが WebSocket で **ProjectCommand** をリアルタイム同期する。テスト実行 UI は Phase 1 と **同一**（`@qarows/runner-ui`）。
...
| 実行 UI | `@qarows/runner-ui` | Phase 1 と同一のランナー体験 |
...
Phase 2 のリアルタイム同期は **JSON Patch ではなく `ProjectCommand`** を送受信する。
...
Phase 1 のファイルマージルール（OK &lt; SKIP &lt; NG）は **Phase 1 専用**（`mergeResults` command）。Phase 2 の同時編集は server revision 付き command 適用で整合する。
